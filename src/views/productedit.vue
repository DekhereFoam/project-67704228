<template>
      ...
      <tbody>
        <tr v-for="product in products" :key="product.product_id">
          ...
          <td>
            <img
              v-if="product.image"
              :src="'http://localhost:8081/project-67704228/api_php/upload/' + product.image"
              width="100"
            />
          </td>
          <td>
  ...
  <div v-if="isEditMode && editForm.image">
    ...
    <img
      :src="'http://localhost:8081/project-67704228/api_php/upload/' + editForm.image"
      width="100"
    />
  </div>
</div>
...
export default {
  ...
  setup() {
    ...
    const fetchProducts = async () => {
      try {
        const res = await fetch("http://localhost:8081/project-67704228/api_php/show_product.php");
...
// ✅ ใช้ฟังก์ชันเดียวในการเพิ่ม / แก้ไข
    const saveProduct = async () => {
      ...
      try {
        const res = await fetch("http://localhost:8081/project-67704228/api_php/productedit.php", {
          method: "POST",
          body: formData
        });
